            ...
            <el-table ref="adminTable" :data="list" style="width: 100%;" border stripe v-loading="listLoading">
                <el-table-column label="问卷提交者" align="center">
                    <template #default="scope">
                        <div @click="handleSelectNews(scope.row)" class="file-name-content">{{ scope.row.tempName }}___{{ scope.row.userName }}</div>
                    </template>
                </el-table-column>
...
function handleSelectNews(row: any) {
    router.push({ path: '/research/tempView', query: { tempId: row.tempId,userId:row.userId } });
}
