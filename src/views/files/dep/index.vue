...
//获取文件
function getList() {
    list({ pageType: panUtil.fileFold.DEP, parentId: fileStore.parentDepId, fileTypes: fileStore.fileTypes }).then(response => {
...
function handleNodeClick(item: any, data: any) {
    //加载文件
    list({ pageType: panUtil.fileFold.DEP, parentId: item.id, fileTypes: fileStore.fileTypes }).then(response => {
        fileList.value = response.data
    });
    ...
    while (nodeData.parent != null) {
        breadcrumbStore.depBreadCrumbs.unshift({ 'name': nodeData.data.label, 'id': nodeData.data.id });
...
function goToThis(id: any) {
    if (id !== '-1') {
        ...
        // 加载文件
        list({ pageType: panUtil.fileFold.DEP, parentId: id, fileTypes: fileStore.fileTypes }).then(response => {
            fileList.value = response.data
        });
    }
}
...
function goInFolder(item: any) {
    ...
    //加载文件
    list({ pageType: panUtil.fileFold.DEP, parentId: item.fileId, fileTypes: fileStore.fileTypes }).then(response => {
        fileList.value = response.data
    });
    //面包屑
    let breadItem = {
...
function showMusic(row: any) {
    openNewPage('/preview/music/' + row.parentId + '/' + row.fileId + '/' + panUtil.fileFold.DEP, 'PreviewMusic', {
        pageType: panUtil.fileFold.DEP,
        parentId: row.parentId,
        fileId: row.fileId
    })
