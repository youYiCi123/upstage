<template>
    <div class="upload-button-content">
        <el-button v-if="roundFlag" :icon="Upload" id="uploadButton" :size="size" @click="doDownloadFileFlag">
            上传
        </el-button>
...
import { secUpload, merge, getAuth } from '../../../api/file'
...
function doDownloadFileFlag() {
    getAuth({ fileId: fileStore.topFileId, userId: userStore.id }).then((res) => {
        if(res.data==null||(res.data!=null&&res.data.uploadFileFlag==1)){
            uploadDialogVisible.value = true
        }else{
            ElMessage.error('你没有上传文件的权限，请联系管理员')
        }
    })
}
